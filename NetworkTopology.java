...
public class NetworkTopology {
  ...
  public Node[] getChildren(Node parent)
  {
	  ...
	  InnerNode innerNode = (InnerNode)parent;
	  int childnum = innerNode.getNumOfLeaves();
	  List<Node> children = new ArrayList<Node>();
	  for(int i = 0;i<childnum;i++)
	  {
		  Node node = innerNode.getLeaf(i, parent);
		  children.add(node);
	  }
	  return children.toArray(new Node[childnum]);
  }
